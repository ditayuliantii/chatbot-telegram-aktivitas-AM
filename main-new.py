...
def process_customer_feedback(message):
    if message.text.lower() == 'ya':
        ...
        user_data['customer_feedback'] = None
...
def process_activity_no_project(message):
    activity_options = ["Kunjungan", "Rapat", "Survey", "Submit Penawaran", "Negosiasi", 
                        ...
                        "Penanganan Gangguan dan Isolir", "Penanganan Invoice", "Lainnya"]
    markup = telebot.types.ReplyKeyboardMarkup(one_time_keyboard=True)
...
def process_manual_activity_no_project(message):
    user_data['activity'] = message.text
    ask_date_start(message)
...
def confirm_and_save(message):
    ...
    summary = (
        ...
        f"Aktivitas: {user_data['activity']}\n"
        f"Nama Pelanggan: {user_data.get('customer_name', '-')}\n"
        f"ID Proyek: {user_data.get('project_id', '-')}\n"
        ...
        f"Kontak Pelanggan: {user_data.get('customer_contact', '-')}\n"
        f"Suara Pelanggan: {user_data.get('customer_feedback', '-')}\n"
        f"Lampiran: {user_data.get('attachment', '-')}\n"
    )
...
def save_data_to_json(user_id, username):
    ...
    activity_record = {
        ...
        'customer_name': user_data.get('customer_name', None),
        'project_id': user_data.get('project_id', None),
        'start_date': user_data['start_date'],
        'start_time': user_data['start_time'],
        'end_date': user_data['end_date'],
        'end_time': user_data['end_time'],
        'notes': user_data['notes'],
        'kontak_pelanggan': user_data.get('customer_contact', None),
        'suara_pelanggan': user_data.get('customer_feedback', None),
