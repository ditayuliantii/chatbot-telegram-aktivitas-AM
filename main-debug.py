...
# def create_markup_with_columns(items, callback_prefix):
#     markup = InlineKeyboardMarkup()
#     row = []
#     for item in items:
#         row.append(InlineKeyboardButton(str(item), callback_data=f"{callback_prefix}_{item}"))
#         if len(row) == 10:  # Setiap baris memiliki 5 kolom
#             markup.add(*row)
#             row = []
#     if row:  # Menambahkan sisa jika ada
#         markup.add(*row)
#     return markup

# # Meminta input tanggal mulai kegiatan dengan inline keyboard
# def ask_date_start(message):
#     days = range(1, 32)  # Sesuaikan untuk tanggal (1-31)
#     markup = create_markup_with_columns(days, "start_day")
...
# # Meminta input bulan mulai kegiatan
# def ask_start_month(message):
#     months = range(1, 13)  # 1-12 untuk bulan
#     markup = create_markup_with_columns(months, "start_month")
...
#     years = range(current_year, current_year + 5)  # Rentang tahun
#     markup = create_markup_with_columns(years, "start_year")
...
# # Meminta waktu mulai kegiatan (jam dan menit)
# def ask_start_time(message):
#     hours = range(0, 24)
#     markup = create_markup_with_columns(hours, "start_hour")
...
#     minutes = range(0, 60, 5)  # Rentang tiap 5 menit
#     markup = create_markup_with_columns(minutes, "start_minute")
...
# # Meminta input tanggal berakhir kegiatan dengan inline keyboard
# def ask_end_date(message):
#     days = range(1, 32)  # Sesuaikan untuk tanggal (1-31)
#     markup = create_markup_with_columns(days, "end_day")
...
# # Meminta input bulan berakhir kegiatan
# def ask_end_month(message):
#     months = range(1, 13)  # 1-12 untuk bulan
#     markup = create_markup_with_columns(months, "end_month")
...
#     current_year = datetime.now().year
#     years = range(current_year, current_year + 5)  # Rentang tahun
#     markup = create_markup_with_columns(years, "end_year")
...
# # Meminta waktu berakhir kegiatan (jam dan menit)
# def ask_end_time(message):
#     hours = range(0, 24)
#     markup = create_markup_with_columns(hours, "end_hour")
...
#     minutes = range(0, 60, 5)  # Rentang tiap 5 menit
#     markup = create_markup_with_columns(minutes, "end_minute")
...
# Fungsi untuk membuat markup dengan kolom
def create_markup_with_columns(items, callback_prefix):
    markup = InlineKeyboardMarkup()
    row = []
    for item in items:
        row.append(InlineKeyboardButton(str(item), callback_data=f"{callback_prefix}_{item}"))
        if len(row) == 10:  # Setiap baris memiliki 10 kolom
            markup.add(*row)
            row = []
    if row:  # Menambahkan sisa jika ada
        markup.add(*row)
...
# Meminta input tanggal mulai kegiatan dengan 10 kolom per baris
def ask_date_start(message):
...
#         markup.add(InlineKeyboardButton(month, callback_data=f"start_month_{i:02d}"))  # Disimpan sebagai angka 2 digit
#     bot.send_message(message.chat.id, 'Pilih bulan mulai kegiatan:', reply_markup=markup)
...
# # Meminta waktu mulai kegiatan (jam)
# def ask_start_time(message):
#     hours = range(0, 24)
#     markup = create_markup_with_columns(hours, "start_hour")
#     bot.send_message(message.chat.id, 'Pilih jam mulai kegiatan:', reply_markup=markup)

# Meminta input jam mulai kegiatan dengan 10 kolom per baris
def ask_start_time(message):
...
# # Meminta waktu mulai kegiatan (menit)
# def ask_start_minute(message):
#     minutes = range(0, 60, 5)  # Rentang tiap 5 menit
#     markup = create_markup_with_columns(minutes, "start_minute")
#     bot.send_message(message.chat.id, 'Pilih menit mulai kegiatan:', reply_markup=markup)
...
# # Meminta input tanggal berakhir kegiatan
# def ask_end_date(message):
#     days = range(1, 32)  # Sesuaikan untuk tanggal (1-31)
#     markup = create_markup_with_columns(days, "end_day")
#     bot.send_message(message.chat.id, 'Pilih tanggal berakhir kegiatan:', reply_markup=markup)

# Meminta input tanggal berakhir kegiatan dengan 10 kolom per baris
def ask_end_date(message):
    days = range(1, 32)  # Tanggal dari 1 hingga 31
    markup = InlineKeyboardMarkup(row_width=8)  # Atur jumlah kolom per baris menjadi 10
...
#     markup = InlineKeyboardMarkup()
#     for i, month in enumerate(MONTHS, start=1):
#         markup.add(InlineKeyboardButton(month, callback_data=f"end_month_{i:02d}"))  # Disimpan sebagai angka 2 digit
#     bot.send_message(message.chat.id, 'Pilih bulan berakhir kegiatan:', reply_markup=markup)
...
# # Meminta waktu berakhir kegiatan (jam)
# def ask_end_time(message):
#     hours = range(0, 24)
#     markup = create_markup_with_columns(hours, "end_hour")
#     bot.send_message(message.chat.id, 'Pilih jam berakhir kegiatan:', reply_markup=markup)

# Meminta input jam berakhir kegiatan dengan 10 kolom per baris
def ask_end_time(message):
    hours = range(0, 24)  # Jam dari 0 hingga 23
    markup = InlineKeyboardMarkup(row_width=8)  # Atur jumlah kolom per baris menjadi 10
...
# # Meminta waktu berakhir kegiatan (menit)
# def ask_end_minute(message):
#     minutes = range(0, 60, 5)  # Rentang tiap 5 menit
#     markup = create_markup_with_columns(minutes, "end_minute")
#     bot.send_message(message.chat.id, 'Pilih menit berakhir kegiatan:', reply_markup=markup)

# Meminta input menit berakhir kegiatan dengan 10 kolom per baris
def ask_end_minute(message):
    minutes = range(0, 60, 5)  # Rentang menit tiap 5 menit (0, 5, 10, ..., 55)
    markup = InlineKeyboardMarkup(row_width=8)  # Atur jumlah kolom per baris menjadi 10
...
# Meminta input tanggal mulai kegiatan
# def ask_date_start(message):
#     bot.send_message(message.chat.id, 'Masukkan tanggal mulai kegiatan (DD-MM-YYYY).')
#     bot.register_next_step_handler(message, process_date_start)

# # proses tanggal mulai
# def process_date_start(message):
#     date_text = message.text
#     try:
#         valid_date = datetime.strptime(date_text, "%d-%m-%Y")
#         user_data['start_date'] = date_text
#         bot.send_message(message.chat.id, 'Silakan masukkan jam mulai kegiatan (contoh: 09:00).')
#         bot.register_next_step_handler(message, process_start_time)
#     except ValueError:
#         bot.send_message(message.chat.id, 'Format tanggal salah atau tidak valid! Silakan masukkan tanggal dalam format: \n\nDD-MM-YYYY.')
#         bot.register_next_step_handler(message, process_date_start)
...
#         else:
#             bot.send_message(message.chat.id, 'Jam atau menit tidak valid! Silakan masukkan jam mulai kegiatan (contoh: 09:00).')
#             bot.register_next_step_handler(message, process_start_time)
#     else:
#         bot.send_message(message.chat.id, 'Format jam salah! Silakan masukkan jam mulai kegiatan (contoh: 09:00).')
#         bot.register_next_step_handler(message, process_start_time)

# # meminta input tanggal berakhir 
# def ask_end_date(message):
#     bot.send_message(message.chat.id, 'Silakan masukkan tanggal berakhir kegiatan (DD-MM-YYYY).')
#     bot.register_next_step_handler(message, process_end_date)

# # proses tanggal berakhir
# def process_end_date(message):
#     date_text = message.text
#     try:
#         valid_date = datetime.strptime(date_text, "%d-%m-%Y")
#         user_data['end_date'] = date_text
#         bot.send_message(message.chat.id, 'Silakan masukkan jam berakhir kegiatan (contoh: 17:00).')
#         bot.register_next_step_handler(message, process_end_time)
#     except ValueError:
#         bot.send_message(message.chat.id, 'Format tanggal salah atau tidak valid! Silakan masukkan tanggal dalam format: \n\nDD-MM-YYYY.')
#         bot.register_next_step_handler(message, process_end_date)

# # proses waktu berakhir 
...
#             ask_notes(message)
#         else:
#             bot.send_message(message.chat.id, 'Jam atau menit tidak valid! Silakan masukkan jam berakhir kegiatan (contoh: 17:00).')
#             bot.register_next_step_handler(message, process_end_time)
#     else:
#         bot.send_message(message.chat.id, 'Format jam salah! Silakan masukkan jam berakhir kegiatan (contoh: 17:00).')
#         bot.register_next_step_handler(message, process_end_time)
